...
#' @param cluster A character string specifying the cluster labels of cells.
...
muscat_estimation <- function(SCST_Object,
  ...
  ## cluster
  if(is.null(cluster)){
    cluster <- rep("A", ncol(ref_data))
  }

  col_data <- data.frame("sample_id" = cluster,
...
muscat_simulation <- function(estimated_result,
  ...
  if(nGroups == 1){
    ...
    existed_params[["ns"]] <- 2
    existed_params[["dd"]] <- TRUE
    existed_params[["paired"]] <- TRUE
  }
  ## phylo_pars
